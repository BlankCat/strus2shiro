(function($) {
	$.fn.Scroll = function() {
		var _btnUp = $("#btnDown"); // 向上按钮
		var _btnDown = $("#btnUp"); //向下按钮
		...
		function setParentMenuTop(c){
			for(var i=1;i<=count;i++){
				...
				var currHeight = i * lineH - (c+1) * lineH + $("#btnDown").parent().height();
				if(currHeight + height > bodyHeight) {
					$(".secondFLoat"+i).css("bottom","0px");
					$(".secondFLoat"+i).css("top","auto");
				} else {
				$(".secondFLoat"+i).css("top",currHeight+"px");
				}
			}
		}
		function scrollDown() {

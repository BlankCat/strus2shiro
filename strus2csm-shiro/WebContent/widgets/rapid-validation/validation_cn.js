...
Validator.messageSource['en-us'] = [
	...
	['validate-date-time' , 'Please use this date format: %s. For example %s.'],
...
Validator.messageSource['zh-cn'] = [
	...
	['validate-date-time' , '请输入有效的日期,格式为 %s. 例如:%s.'],
...
ValidationUtils = {
	...
	isDateTime : function(v,dateFormat) {
		var MONTH = "MM";
	   	var DAY = "dd";
	   	var YEAR = "yyyy";
	   	var HOUR = "hh";
	   	var MINUTE = "mm";
	   	var SECOND = "ss";
		var regex = '^'+dateFormat.replace(YEAR,'\\d{4}').replace(MONTH,'\\d{2}').replace(DAY,'\\d{2}').replace(HOUR,'\\d{2}').replace(MINUTE,'\\d{2}').replace(SECOND,'\\d{2}')+'$';
		if(!new RegExp(regex).test(v)) return false;
		var year = v.substr(dateFormat.indexOf(YEAR),4);
		var month = v.substr(dateFormat.indexOf(MONTH),2);
		var day = v.substr(dateFormat.indexOf(DAY),2);
		var hour = v.substr(dateFormat.indexOf(HOUR),2);
		var minute = v.substr(dateFormat.indexOf(MINUTE),2);
		var second = v.substr(dateFormat.indexOf(SECOND),2);
		var d = new Date(ValidationUtils.format('%s/%s/%s %s:%s:%s',[year,month,day,hour,minute,second]));
		return ( parseInt(month, 10) == (1+d.getMonth()) ) && 
					(parseInt(day, 10) == d.getDate()) && 
					(parseInt(year, 10) == d.getFullYear() )&&
					(parseInt(hour, 10) == d.getHours() )&&
					(parseInt(minute, 10) == d.getMinutes()) &&
					(parseInt(second, 10) == d.getSeconds());	
	},
...
Validation.addAllThese([
	...
	/*
	 * Usage: validate-date-$dateFormat or validate-date($dateFormat default is yyyy-MM-dd hh:mm:ss)
	...
	['validate-date', function(v,elm,args,metadata) {
		...
		['validate-date-time', function(v,elm,args,metadata) {
			var dateFormat = args.singleArgument || 'yyyy-MM-dd hh:mm:ss';
			metadata._error = ValidationUtils.format(ValidationUtils.getI18nMsg(metadata.className),[dateFormat,dateFormat.replace('yyyy','2006').replace('MM','03').replace('dd','12').replace('hh','12').replace('mm','12').replace('ss','12')]);
			return ValidationUtils.isDateTime(v,dateFormat);
		}],
	...
	['validate-phone',/^(\d{3,4}-\d{7,8})(-\d+)?|\d{11}$/],

...
(function () {
    ...
    function insertSearch( id, catchRemote ) {
    	var div=parent.document.getElementById("uploadFiles"); 
    	var uploadName='';
        ...
        for ( var i = 0, ci; ci = imgs[i++]; ) {
            if ( ci.getAttribute( "selected" ) ) {
                ...
                originalFileName=url.substr(url.lastIndexOf("/")+1,url.length);
                uploadName=url.substr(url.lastIndexOf("/")+1,url.length);
                var fileName=document.createElement("input"); 
               	fileName.value=uploadName;
               	fileName.name="attachmentFilenames";
               	fileName.style.display='none';
               	div.appendChild(fileName); 
               	var attachmentPath=document.createElement("input");
               	attachmentPath.value=originalFilePath;
               	attachmentPath.name="attachmentPaths";
               	attachmentPath.style.display='none';
               	div.appendChild(attachmentPath); 
               	var attachmentName=document.createElement("input");   
               	attachmentName.value=originalFileName;   
               	attachmentName.name="attachmentNames";
               	attachmentName.style.display='none';
               	div.appendChild(attachmentName); 
            }
        }
        insertImage(imgObjs);
    ...
    function insertBatch() {
        ...
        for ( var i = 0, ci; ci = imageUrls[i++]; ) {
            ...
            imgObjs.push( tmpObj );
            uploadName=ci.title;
            originalFilePath=ci.url;
            originalFileName=ci.url.substr(ci.url.lastIndexOf("/")+1,ci.url.length);
       //     alert(uploadName);
         //   alert(originalFilePath);
                            ...
                            onsuccess:function ( xhr ) {
                                ...
                                for ( var k = 0, ci; ci = imageUrls[k++]; ) {
                                	(function(i){
                                        ...
                                        a.style.display = "none";
                                        a.onclick = function(){
                                        	if (confirm('如果该图片已被文章或者其他内容引用，删除该图片后所引用位置将不再显示该图片。确认删除吗？')) {
                                        		var path=$(this).parent().find('img').attr('src');
                                        		$.post('../../../admin/content/v_delImg.do',{'imgPath':path},function(){
                                        		});
                                        		$(div).remove();
											}
                                        };

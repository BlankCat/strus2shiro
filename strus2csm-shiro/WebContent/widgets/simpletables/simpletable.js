...
var SimpleTable = function(formId,pageNumber,pageSize,sortColumns,pageNumberKey,pageSizeKey,sortColumnsKey) {
	...
	$("#"+formId+" .gridBody th[sortColumn]").click(function() {
		...
		var tit=$(this).attr('titc');
		$(this).toggleClass('tableHeaderSortHover',true);
		layer.tips(tit, this, {
			guide: 2,
			time: 1,
			style: ['background-color:#feffd6; color:#bc7b08', '#feffd6'],
			maxWidth:150
		});
	}).mouseout(function() {
	...
	for(var i = 0; i < sortInfos.length; i++) {
		...
		var selectall="#"+formId+' .gridBody th';
		var tit=$(selector).attr('titc');
		$(selectall).addClass('sort da');
		var order = info.order ? info.order : 'asc';
		$(selector).addClass("sort " + order.toLowerCase());
		layer.tips(tit, selector, {
			guide: 2,
			time: 2,
			style: ['background-color:#feffd6; color:#bc7b08', '#feffd6'],
			maxWidth:240
...
SimpleTable.prototype = {
	doJump : function(pageNumber,pageSize,sortColumns) {
		/*alert("pageNumber:"+pageNumber+" pageSize:"+pageSize+" sortColumns:"+sortColumns+" this.form:"+thisform);*/
